  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        neoWrong: cn(neoClasses, "bg-red-500 text-primary-foreground border-red-600")
      },
      size: {
  ...
  ["absolute","h-14","bottom-[-7px]","w-full","border-2","left-0","z-0"],{
    variants:{
      variant: {
        ...
        neoWrong: "border-red-500 bg-red-600"
      },
  },
  defaultVariants:{
    variant: "default"
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    const containerClasses = clsx({
      relative: variant === "neo" || variant ==="neoOutline" || variant ==="neoCorrect" || variant==="neoWrong"
    });

    const borderRound = clsx({
      "rounded-full": variant === "neo",
      "rounded-2xl": variant === "neoOutline" ||variant ==="neoCorrect" || variant ==="neoWrong"
    })
